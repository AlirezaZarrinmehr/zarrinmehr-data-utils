...
def process_ns_transactions(
    ...
    txnsLines = transactionLine.copy()
    generalJournal = transaction.loc[transaction['type'].isin(['Journal'])].copy()
    generalJournalLines = transactionLine[(transactionLine['transaction'].isin(generalJournal['id'].dropna().unique()))].copy()
    generalJournalLines = generalJournal.merge(generalJournalLines, left_on = 'id', right_on = 'transaction', suffixes = ('_h', ''))
    ...
    generalJournalLines['id'] = generalJournalLines['index']
    generalJournalLines['transaction'] = generalJournalLines['index']
    generalJournalLines['foreigntotal'] = generalJournalLines['foreignamount']
    generalJournalLines['custbody_stc_total_after_discount'] = generalJournalLines['foreignamount']
    ...
    txns = txns[
        (pd.to_datetime(txns['trandate'])>=start_date)&\
        (pd.to_datetime(txns['trandate'])<=end_date)
    ].copy() 
    ...
    txns['entity'] = txns['entity'].fillna('').apply(convert_to_int_or_keep).astype('str')
    customer['CustId'] = customer['CustId'].fillna('').apply(convert_to_int_or_keep).astype('str')
    txns = txns.merge(customer[['CustId', 'CustNo', 'CustName', 'SalesRepID']], left_on = 'entity', right_on = 'CustId', how = 'left')
    txns = txns.merge(employee[['id', 'Name']].rename(columns = {'id':'employeeId', 'Name':'SalesRepID2'}).drop_duplicates(subset = ['employeeId']), left_on = 'custbodycustbodycsr_assigned', right_on = 'employeeId', how = 'left')
    txns['typebaseddocumentnumber']=txns['typebaseddocumentnumber'].fillna(txns['number'].fillna('').apply(convert_to_int_or_keep).astype('str'))
    txns.rename(columns = {
        'id':'TransactionId',
        'otherrefnum':'CustPo',
        'typebaseddocumentnumber':'TransactionNo',
        'type':'TransactionType',
        'trandate':'TransactionDate',
        'closedate':'CloseDate',
        'billingstatus':'TransactionStatus',
        'shipdate':'ShipDate',
    ...
    txnsLines = txnsLines[(txnsLines['transaction'].isin(txns['TransactionId'].dropna().unique()))].copy()
    remove_accounts = set(
        ['122', '257']
    )
    txnsLines = txnsLines[
        (txnsLines['subsidiary'].fillna('').astype('str').str.strip()==subsidiaryId)&\
        (~txnsLines['expenseaccount'].astype('str').isin(remove_accounts))
    ].copy()
    ...
    txnsLines.loc[txnsLines['itemtype']=='TaxGroup', 'ItemId']=-8
    txnsLines.ItemId = txnsLines.ItemId.fillna('').apply(convert_to_int_or_keep).astype('str')
    item.ItemId = item.ItemId.fillna('').apply(convert_to_int_or_keep).astype('str')
    txnsLines = txnsLines.merge(item[['ItemId', 'ItemNo', 'ItemName']], on='ItemId', how='left')
    txnsLines = txnsLines.merge(txns[['TransactionId', 'TransactionNo']], on='TransactionId', how='left')
    ...
    txns.drop(columns = ['Total'], inplace = True)
    txns = txns.merge(
    ...
    txns = txns[~txns['TransactionId'].isin(mismatched_txns['TransactionId'].dropna().unique())]
    txnsLines['Company'] = companyName
    txnsLines = txnsLines[['Company'] + txnsLines.columns[:-1].tolist()]
    txnsLines = txnsLines[txnsLines['TransactionId'].isin(txns['TransactionId'].dropna().unique())]
    txnsLines=txnsLines.merge(accounts[['fullname','AccountType']].drop_duplicates(subset=['fullname']).rename(columns={'fullname':'Account'}), on='Account', how = 'left')
    return txns, txnsLines
...
def process_gp_transactions(
    ...
    generalLedgerLineOpen = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name, object_key = 'GL20000 :: YEAR-TO-DATE TRANSACTION OPEN.csv')
...
def process_qb_transactions(
    ...
    generalJournal = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'JournalEntry.csv')
    generalJournalLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'JournalEntryLine.csv')
    ...
    generalJournalLines.rename(columns = {
        ...
        'JournalLineEntityRefFullName': 'CustNo',
    ...
    bills = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'Bill.csv')
    ...
    billExpenseLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'BillExpenseLine.csv')
    ...
    checks = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'Check.csv')
    ...
    checkExpenseLine = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'CheckExpenseLine.csv')
    ...
    checkExpenseLine['Total'] = -checkExpenseLine['Total']
    ...
    receivePayment = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'ReceivePayment.csv')
    ...
    receivePaymentLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'ReceivePaymentLine.csv')
    ...
    ] = receivePaymentLines['DepositToAccountRefFullName']
    receivePaymentLines.loc[
        receivePaymentLines['AppliedToTxnDiscountAccountRefFullName'].isna(),
        'AppliedToTxnDiscountAmount'
    ] = receivePaymentLines['TotalAmount']
    receivePaymentLines.rename(columns = {
        ...
        'TxnID': 'TransactionId',
        'RefNumber': 'TransactionNo',
        'PaymentMethodRefFullName': 'ItemId',
        'Memo': 'ItemDescription'
    }, inplace = True)
    ...
    deposit = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'Deposit.csv')
    depositLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'DepositLine.csv')
    ...
    depositLines.rename(columns = {
        ...
        'Memo': 'OrderNo',
        'DepositLineEntityRefFullName': 'CustNo',
    ...
    invoice = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'Invoice.csv')
    ...
    invoiceLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'InvoiceLine.csv')
    ...
    billsLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'BillItemLine.csv')
    ...
    checksLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'CheckItemLine.csv')
    ...
    vendorCredit = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'VendorCredit.csv')
    ...
    vendorCreditLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'VendorCreditItemLine.csv')
    ...
    vendorCreditExpenseLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'VendorCreditExpenseLine.csv')
    ...
    salesReceipts = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'SalesReceipt.csv')
    ...
    salesReceiptsLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'SalesReceiptLine.csv')
    ...
    billPaymentCheck = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'BillPaymentCheck.csv')
    ...
    billPaymentCheckLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'BillPaymentCheckLine.csv')
    ...
    creditMemo = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'CreditMemo.csv')
    ...
    creditMemoLines = read_file_from_s3(s3_client = s3_client, bucket_name = s3_bucket_name_bronze, object_key = 'CreditMemoLine.csv')
    ...
    creditMemoLines.rename(columns = {
        ...
        'RefNumber': 'TransactionNo',
        'AccountRefFullName': 'Account',
        'CreditMemoLineItemRefFullName': 'ItemId',
        'CreditMemoLineDesc': 'ItemDescription',
    ...
    txns['CustNo'] = txns['CustNo'].fillna('').astype('str')
    customersORvendors['CustNo'] = customersORvendors['CustNo'].fillna('').astype('str')
    txns = txns.merge(customersORvendors[[f'CustId', 'CustNo', f'CustName']], on = 'CustNo', how = 'left').copy()
    ...
    txns = txns[['OrderNo', 'TransactionId', 'TransactionNo', 'TransactionStatus', 'TransactionType', 'TransactionDate', 'SalesRepID', 'CustPo', f'CustId', 'CustNo', f'CustName', 'ShipName', 'ShipAddress', 'ShipCity', 'ShipState', 'ShipZip', 'BillName', 'BillAddress', 'BillCity', 'BillState', 'BillZip', 'subTotal', 'Total']].copy()
    txns['Company'] = companyName
    txns = txns[['Company'] + txns.columns[:-1].tolist()]
    txnsLines = txnsLines[txnsLines['TransactionId'].isin(txns['TransactionId'])]
    return txns, txnsLines
...
def process_qb_orders(
    ...
    SalesRep.ListID = SalesRep.ListID.fillna('').astype('str')
    orders = orders.merge(
        SalesRep[['ListID', 'SalesRepEntityRefFullName']].drop_duplicates(subset=['ListID']).rename(columns = {'ListID': object_key_4, 'SalesRepEntityRefFullName': txnsType4}),
        on=object_key_4,
        how='left',
        suffixes=('', '_')
    )
    ...
    if companyName == 'KOMAR':  
        ...
        shipInstallDates = read_file_from_s3(s3_client = s3_client, bucket_name = 'manual-db', object_key = 'serialNumberEngineeringReferenceMaster.csv')[['Serial Number', 'Installation Date']].rename(columns = {'Serial Number' : 'SerialNo', 'Ship Date' : 'ShipDate', 'Installation Date' : 'InstallDate'})
...
def process_s50_transactions(
    ...
    log_message(f'[INFO] {mismatched_txns.shape[0]} txns Total do not match orderline Total')
    txns = txns[~txns['TransactionId'].isin(mismatched_txns['TransactionId'])]
...
def process_s50_orders(
    ...
    #------------------------------------------------------------
    log_message(f'[INFO] Step5: {txnsType}...')
    #------------------------------------------------------------
...
def process_exchange_rates(
    ...
    try:
        df = read_file_from_s3(s3_client = s3_client, bucket_name = bucket_name, object_key = object_key)
        df['Date']= pd.to_datetime(df['Date'], errors='coerce')
    ...
    try:
        ...
        with tqdm(total=total_chunks, desc="Fetching data from MS SQL") as pbar:
            while True:
                ...
                if not rows:
                    break
                yield [dict(zip(columns, row)) for row in rows], columns
                pbar.update(1)
    finally:
        cursor.close()
...
def _stream_qodbc(sql_query, conn, chunksize):
    ...
    try:
        ...
        with tqdm(desc="Fetching data from QODBC", unit="chunk") as pbar:
            while True:
                rows = cursor.fetchmany(chunksize)
                if not rows:
                    break
...
def _stream_qboapi(sql_query, qbo_token_path, client_id, client_secret, redirect_uri, environment, chunksize):
    ...
    with tqdm(desc="Fetching data from QuickBooks", unit="chunk") as pbar:
        while has_more:
            ...
            table_name = next(iter(query_response.keys()), None)
            table = query_response.get(table_name, [])
            if not table:
                has_more = False
                if first_loop:
                    yield [], ["Id", "Metadata.Lastupdatedtime"]
                break
            if table_name == 'RecurringTransaction':
                rows = []
                for record in table:
                    txn_type = list(record.keys())[0]
                    flat = pd.json_normalize(record[txn_type])
...
def _stream_bigquery(sql_query, project_id, credentials, chunksize):
    ...
    with tqdm(desc="Fetching data from BigQuery", unit="chunk") as pbar:
        for chunk_df in df_iterator:
            if chunk_df.empty:
                continue
            yield chunk_df.to_dict(orient='records'), list(chunk_df.columns)
            pbar.update(1)
    ...
    try:
        ...
        if file_path:
            ...
            with open(file_path, 'w', newline='', encoding=encoding) as f:
                writer = csv.writer(f)
                for chunk_data, columns in data_generator:
                    if first_chunk:
...
def generate_open_cases_df(
    ...
    log_message(f'[INFO] Generated open cases dataframe successfully!')
    return open_df
...
def train_and_predict(
    ...
    log_message(f'[INFO] Exact match accuracy: {exact_match_acc:.4f}')
...
def get_full_resource(api_url):
    ...
    if total_number is not None:
        ...
        while True:
            response = get_resource(api_url, params)
            resources.extend(response['value'])
            log_message(f'[INFO] Page {page_index} added!')
            try:
                api_url = response['@odata.nextLink']
                page_index += 1
                time.sleep(1)
            except:
                log_message(f'[INFO] All pages retrieved!')
                break
...
def list_timestream_tables(timestream_write_client, database_name):
    try:
        ...
        log_message(f'[ERROR] Error listing tables in {database_name}: {e}')
        return []
...
def get_ewon_details(base_url, developer_id, encodedName, device_username, device_password, account, username, password, session_id=None):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # log_message(f'[INFO] Get Ewon Details Successful!')
            # return parsed_data
        else:
            raise Exception()
    except:    
        # log_message(f'[INFO] Get Ewon Details Failed: {response.text}')
        return {}
...
def stop_driver(ip_address, username, password):
    ...
    try:
        ...
        if response.status_code == 200 and response.text.strip() == "JVM Stopped":
            ...
            log_message(f'[INFO] - Status Code: {response.status_code}')
            log_message(f'[INFO] - Response: {response.text}')
            return False
...
def enrich_and_classify_customers(
    ...
    if DBIA:
        ...
        dfCategories = read_file_from_s3(s3_client = s3_client, bucket_name = 'manual-db', object_key = cat_object_key)
        ...
        log_message(f'[INFO] Found: {dfCategories.Found.sum()}...')
        return df
...
def generate_table_select_queries(
    ...
    if datasets:
        ...
        log_message(f'[INFO] {project} project does not contain any datasets.')
    if tables_to_remove:
...
def wait_for_cluster_available(
    ...
    try:
        log_message(f'[INFO] Waiting for the Redshift cluster "{redshift_cluster_identifier}" to become available...')
        ...
        if cluster_status == 'available':
            log_message(f'[INFO] Cluster "{redshift_cluster_identifier}" is now available.')
        else:
            log_message(f'[INFO] Cluster "{redshift_cluster_identifier}" is not available. Current status: {cluster_status}')
...
def attach_policies_to_role(
    ...
    for policy in role_policies:
        try:
            ...
            log_message(f'[INFO] Policy "{policy}" attached to role "{role_name}".')
...
def associate_role_with_redshift(
    ...
    try:
        ...
        if role_associated:
            ...
            log_message(f'[INFO] Attempting to associate the role "{redshift_iam_role_arn}" with the Redshift cluster "{redshift_cluster_identifier}".')
            ...
            while elapsed_time < timeout and not role_associated:
                ...
                if role_associated:
                    ...
                    log_message(f'[INFO] ⏳ Waiting for IAM role "{redshift_iam_role_arn}" to be associated with the Redshift cluster "{redshift_cluster_identifier}". Retrying...')
...
def add_inbound_rule(
    ...
    if not rule_exists:
        log_message(f'[INFO] Adding inbound rule for security group to allow access from 0.0.0.0/0...')   
...
def turn_on_case_sensitivity(
    ...
    log_message(f'[INFO] Modified parameter group "{parameter_group_name}"')
    wait_for_cluster_available(redshift_client, redshift_cluster_identifier)
    ...
    if current_parameter_group != parameter_group_name:
        ...
        log_message(f'[INFO] Modified cluster "{redshift_cluster_identifier}" with parameter group "{parameter_group_name}"')
...
def upload_to_redshift(
    ...
    try:
        ...
        if createRedshiftCluster:
            log_message(f'[INFO] Cluster "{redshift_cluster_identifier}" not found. Creating a new one...')
    ...
    for bucket in s3_bucket_names:
        log_message(f'[INFO] Scanning S3 bucket: "{bucket}"')
        try:
